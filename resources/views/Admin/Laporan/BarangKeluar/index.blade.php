                ...
                <div class="table-responsive">
                    <table id="table-1" class="table table-bordered text-nowrap border-bottom dataTable no-footer dtr-inline collapsed">
                        <thead>
                            ...
                            <th class="border-bottom-0">Surat Jalan</th>
                        </thead>
                        <tbody></tbody>
                    </table>
                </div>
...
"columns": [{
    ...
    },
    {
        ...
        render: function(data, type, row) {
            return `<button class="btn btn-sm btn-light" onclick="suratPerTujuan('${row.bk_tujuan}')">
                        <i class="fe fe-printer"></i> Surat Jalan
                    </button>`;
        }
    },
],
    ...
    function suratPerTujuan(tujuan) {
    var tglawal = $('input[name="tglawal"]').val();
    var tglakhir = $('input[name="tglakhir"]').val();
    
    let url = "{{ route('lap-bk.surat') }}?tujuan=" + encodeURIComponent(tujuan);
    ...
    if (tglawal != '' && tglakhir != '') {
        window.open(
            "{{ route('lap-bk.surat') }}?tglawal=" + tglawal + "&tglakhir=" + tglakhir,
            '_blank'
        );
    } else {
        swal({
            title: "Yakin cetak Surat Jalan untuk Semua Data?",
            type: "warning",
            buttons: true,
            dangerMode: true,
            confirmButtonText: "Yakin",
